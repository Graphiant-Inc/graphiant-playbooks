...
- **Verification:**
  - Sanity tests run in CI: `.github/workflows/lint.yml` (lint stage), `.gitlab/pipelines/lint.yml` (lint stage)
...
- **Verification:**
  ...
  - **GitLab CI:** `.gitlab/pipelines/run.yml`
    - Parallel matrix includes:
      - `ANSIBLE_CORE: 2.17` ✅
      - `ANSIBLE_CORE: 2.18` ✅
      - `ANSIBLE_CORE: 2.19` ✅
    - Tests run for each version:
...
- **Verification:**
  - Workflows trigger on `pull_request` events:
    - `.github/workflows/test.yml` ✅ (includes Python tests, validation against multiple ansible-core versions, separate E2E integration test job)
    - `.github/workflows/lint.yml` ✅ (includes djlint, ansible-lint, documentation lint, collection structure validation, ansible-test sanity against multiple versions)
    - `.github/workflows/build.yml` ✅ (runs after test workflow completes)
  - GitLab pipelines trigger on merge requests:
    - `.gitlab/pipelines/run.yml` ✅ (includes Python tests and validation against multiple ansible-core versions, separate E2E integration test job)
    - `.gitlab/pipelines/lint.yml` ✅ (includes djlint, ansible-lint, documentation lint, ansible-test sanity against multiple versions)
    - Pipeline stages run in order: `lint` → `run` → `build` → `publish` ✅
  - All tests run on PRs/MRs ✅
...
- **Verification:**
  ...
  - **GitLab CI:** `.gitlab/pipelines/lint.yml` (ansible-test-sanity job with parallel matrix testing against 2.17, 2.18, 2.19)
  - Both platforms test against multiple ansible-core versions on release commits ✅
...
- **Verification:**
  - **Lint Stage/Workflow:** Focuses on static analysis
    ...
    - Collection structure validation (uses `scripts/validate_collection.py`) ✅
  ...
  - **Test/Run Stage/Workflow:** Focuses on testing and validation
    ...
    - Collection validation (uses `scripts/validate_collection.py`, runs against multiple ansible-core versions: 2.17, 2.18, 2.19) ✅
  ...
  - **Stage Ordering:** In MR/PR pipelines, stages run in order: `lint` → `run` → `build` → `publish` ✅
  - **GitHub Actions:** `.github/workflows/test.yml` and `.github/workflows/lint.yml`
  - **GitLab CI:** `.gitlab/pipelines/run.yml` and `.gitlab/pipelines/lint.yml`
...
- **Jobs:**
  - `test` - Matrix job testing against ansible-core 2.17, 2.18, 2.19:
    - Python unit tests
    - Collection validation (uses `scripts/validate_collection.py`)
...
- **Jobs:**
  - `jinjalint` - Jinja2 template linting (djlint)
  - `ansible-lint` - Ansible playbook best practices
  - `lint-collection-docs` - Documentation linting (antsibull-docs)
  - `ansible-test-sanity` - Parallel matrix job testing against ansible-core 2.17, 2.18, 2.19
- **Stage:** lint

#### `run.yml` - Test Pipeline
- **Purpose:** Testing and validation
- **Jobs:**
  - `test` - Parallel matrix job testing against ansible-core 2.17, 2.18, 2.19:
    - Python unit tests
    - Collection validation (uses `scripts/validate_collection.py`)
  - `e2e-integration-test` - Separate job (not in matrix):
    - E2E integration test (hello_test.yml) - conditional on credentials
