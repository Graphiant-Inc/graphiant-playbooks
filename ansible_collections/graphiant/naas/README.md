...
- Public VIF (gateway "local data exchange") services on Graphiant-managed gateway appliances
...
| `public_vif_management.yml` | Public VIF (local data exchange) services create/update/delete/query |
| `data_assurance_management.yml` | Data Assurance policies configure/deconfigure |
...
ansible-doc graphiant.naas.graphiant_public_vif_info
ansible-doc graphiant.naas.graphiant_gateway_services
...
- **Public VIF services**: `create_services` and `delete_services` skip already-existing/already-absent services (`changed: false`, matched by `serviceName`). `update_services` is a full PUT replace (not a merge/patch — see `sample_public_vif_services_update.yaml`) and does NOT compare against live service state — every run pushes the full desired payload and reports `changed: true`, the same documented behavior as `graphiant_bgp`/`graphiant_device_config` configure operations.
- **Gateway Services**: `create` is an idempotent create-or-update — services that do not exist are created, an existing **connectivity** service is updated in place (PUT) only when its config differs and skipped (`changed: false`) when it already matches, and an existing **cloud gateway** is skipped (update is not supported; change one via delete + create). `delete` skips gateways with no matching existing service. Volatile tunnel fields (`psk`, inside CIDRs, BGP `md5Password`) are excluded from the comparison, so an unchanged connectivity gateway is skipped without re-allocating subnets/keys — which also means a *rotated* PSK/MD5 is not auto-detected; set `force_update: true` to re-push matching connectivity gateways so the new secret takes effect (cloud gateways are never updated). Secrets are redacted as `********` in `--diff`. Use `--check`/`--diff` to preview via `details.diff_plan`.
...
- `sample_public_vif_services.yaml` / `sample_public_vif_services_update.yaml` - Public VIF (local data exchange) service configuration (`public_vif_services` list: producer LAN segment/region/storage provider, consumer LAN segments, gateway BGP neighbors, NAT prefix strategy); every field documented Required/Optional; the update sample adds a second gateway BGP neighbor and notes that `update_services` fully replaces the existing service config
- `sample_data_assurance_policies.yaml` - Data Assurance policy configuration (`DataAssurancePolicies` list for assurance and block-by-URL/app policies; `ContentFilterPolicies` list for block-by-category policies)
